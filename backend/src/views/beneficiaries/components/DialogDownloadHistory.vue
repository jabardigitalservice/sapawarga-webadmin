  ...
  <div>
    <el-table :data="data" stripe>
      ...
      <el-table-column property="name" :label="$t('label.beneficiaries-upload-status')" width="250">
        <template slot-scope="{row}">
          <el-progress :percentage="(isNaN(row.aggregate.total_row_processed * 100 / row.aggregate.total_row_count) === true) ? 0 : parseInt(row.aggregate.total_row_processed * 100 / row.aggregate.total_row_count)" />
        </template>
      </el-table-column>
